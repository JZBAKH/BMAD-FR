...
const FR_WORDS =
  /\b(le|la|les|des|du|de|un|une|et|ou|dans|pour|avec|est|sont|être|avoir|vous|nous|ce|cette|ces|qui|que|dont|si|car|mais|aussi|plus|tous|toute|votre|notre|sur|sous)\b/gi;
...
function renderModuleSection(report) {
  ...
  if (!report.frModuleExists) {
    lines.push(
      `❌ **Pas de version \`-fr\`** — \`src/${report.module}-fr/\` n'existe pas.`,
      '',
      `- ${report.originalFileCount} fichiers à traduire dans l'original`,
      '',
    );
    return lines.join('\n');
  }
  ...
  lines.push(
    `- **Présence** : ${report.presentInFr}/${report.originalFileCount} fichiers (${presencePct})`,
    `- **Traduits effectivement** : ${report.translated.length}/${totalScored} (${translationPct})`,
    `- **Non traduits (copiés mais en anglais)** : ${report.untranslated.length}`,
    `- **Manquants** : ${report.missingInFr.length}`,
  );
...
function renderReport(reports, missingModules, generatedAt) {
  ...
  lines.push(
    ...
    '',
    `_Généré automatiquement le ${generatedAt} par \`tools/audit-translation-coverage.mjs\`._`,
    '',
    '## Résumé global',
    '',
  ...
  for (const r of reports) {
    ...
    lines.push(
      `| \`${r.module}\` | ${r.presentInFr}/${r.originalFileCount} (${presencePct}) | ${translationPct} | ${r.missingInFr.length} | ${status} |`,
    );
  }
...
async function main() {
  ...
  await writeFile(OUT_JSON, JSON.stringify({ generatedAt, missingModules, reports: summary }, null, 2), 'utf8');
