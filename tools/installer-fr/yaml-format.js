...
async function formatYamlContent(content, filename) {
  ...
  try {
    ...
    console.error(chalk.yellow(`💡 Essayez de corriger manuellement la structure YAML d'abord`));
    return null;
  }
}
...
async function main() {
  ...
  if (hasChanges) {
    console.log(chalk.green(`\n✨ Formatage YAML terminé ! ${filesProcessed.length} fichiers modifiés :`));
    for (const file of filesProcessed) console.log(chalk.blue(`  📝 ${file}`));
  }

  if (hasErrors) {
    console.error(chalk.red('\n💥 Certains fichiers ont des erreurs. Veuillez les corriger avant de committer.'));
    process.exit(1);
  }
}
