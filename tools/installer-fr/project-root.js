 ...
 *
 * MODIFICATION FR : redirige les modules built-in (bmm, core) vers
 * leur version traduite -fr. Tout chemin physique passe par cette
 * fonction → garantit que le contenu chargé/copié est en français.
 *
 * Mapping :
 *   - 'core'        → src/core-skills-fr/
 *   - 'bmm'         → src/bmm-skills-fr/
 *   - 'core-skills' → src/core-skills-fr/  (alias direct)
 *   - 'bmm-skills'  → src/bmm-skills-fr/   (alias direct)
 *   - autres modules → src/modules/<name>/ (modules tiers/community)
 */
function getModulePath(moduleName, ...segments) {
  if (moduleName === 'core' || moduleName === 'core-skills') {
    return getSourcePath('core-skills-fr', ...segments);
  }
  if (moduleName === 'bmm' || moduleName === 'bmm-skills') {
    return getSourcePath('bmm-skills-fr', ...segments);
  }
